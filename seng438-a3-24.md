...
A branch and line coverage of 100% for this function is impossible due to code within the function, there is if statement ( if (total > 0) ) that will never be entered due to the variable "total" being set equal to zero directly above the statement. Due to the if statement never resolving to "True", the line of code within the if block, ( total = 100; ), is unreachable.
